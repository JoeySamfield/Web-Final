...
function validate_email() {
	...
	const xhttp_prev_email = new XMLHttpRequest(); 
	xhttp_prev_email.open(
		"GET",
		"prev_email.php?email=" + useremail,
		false
	);
	xhttp_prev_email.send();
	_isNew = xhttp_prev_email.responseText;
...
function validate() {
	if (_prevExists || _badEmail || _badUser || _badPass1 || _badPass2) {
		...
		xhttp_register.send("email=" + useremail + "&user=" + prevuserval + "&pass=" + pass1.value);
		const reg_res = xhttp_register.responseText;
		...
		else alert("Register Failed: "+reg_res);
	}
}